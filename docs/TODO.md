...
- [x] **Fixed 2026-08-25 — index-build failures now fail startup loudly.** `db/ensure-indexes.ts` calls `createIndexes()` on every registered model at boot (which rejects, unlike Mongoose's silent background build) and `server.ts` exits 1 with the offending model and reason. Verified by reproducing the original failure: dropped `users.email`, planted duplicates, restarted — the server refused to start and named the duplicate key, instead of running without the uniqueness guarantee `auth` depends on for its 409. 3 unit tests pin the behaviour.
